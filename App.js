...
function App() {
  ...
  const columns = [
    ...
    { title: "CreatedAt", field: "createdAt", grouping: false, render: rowData => moment(rowData.createdAt).format("DD MMM YY") },
    { title: "UpdatedAt", field: "updatedAt", grouping: false, render: rowData => moment(rowData.updatedAt).format("DD MMM YY") },
    { title: "Price", field: "price", grouping: false },
    { title: "Sale Price", field: "sale_price", grouping: false }
  ];
  ...
  const handleSearchChange = (searchText) => {
    ...
    const fuse = new Fuse(empList, {
      keys: ['name', 'category', 'subcategory'], // Adjust this according to your fields
      threshold: 0.3 // Adjust threshold for sensitivity
    });
    const results = fuse.search(searchText);
    const filteredData = results.map(result => result.item); // Extracting the matched items
    setData(searchText ? filteredData : empList);
  };

  return (
    <div className="App">
      <h1 style={{ fontSize: '5em' }}>Table View</h1>
      <MaterialTable
        data={data}
        columns={columns}
        options={{
          grouping: true,
          search: true, // Disable default search
          paging: true,
          pageSize: 10,
          filtering: true,
          exportButton: true,
        ...
        onSearchChange={handleSearchChange} // Use the fuzzy search function here
      />
    </div>
  );
}
